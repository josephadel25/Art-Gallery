...
$conn = $database->getConnection();
    ...
    <section id="title" class="position-relative py-3 py-lg-7 overflow-hidden" style="padding: 12% 0 5% 0;text-align: center;">
      <ul class="breadcrumb justify-content-center no-border mb-0">
        ...
        <li class="breadcrumb-item text-black active">category</li>
      </ul>
        <div class="title" >
          <h1 class="hero-heading mb-0" style="color: #000000;">ALL PRODUCTS</h1>
        </div>
    </section>
    <?php 
$database = Connect::getInstance();
...
// If search term is provided and category is "Art"
if (!empty($search) && $category === 'art') {
  // Use LIKE in the query to search for artworks by title
  $Qsearch = "SELECT artworks.*,users.name as name
              FROM artworks 
              LEFT JOIN users ON artworks.artistid = users.id
              WHERE artworks.title LIKE '%$search%'";
}
// If search term is provided and category is "Artist"
elseif (!empty($search) && $category === 'artist') {
  // Use LIKE in the query to search for artist by first name
  $Qsearch = "SELECT artworks.*,users.name as name
              FROM artworks 
              LEFT JOIN users ON artworks.artistid = users.id
  ...
  // If no search term is provided or category is not selected, retrieve all artworks
  $Qsearch = "SELECT artworks.*,users.name as name
              FROM artworks 
              LEFT JOIN users ON artworks.artistid = users.id";
}

// Execute the query to retrieve artworks data
$result = $conn->query($Qsearch);
?>
<main>
